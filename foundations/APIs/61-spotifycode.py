...
# prints out data messily
print(data)

# prints out the type of data
print(type(data))

# try to get the keys out
print(data.keys())
# need to use parenthesis even if they are empty if it is a built in method
# keys will not work properly without parenthesis
# apparently it only has one key , called artists

print(data['artists'])
print(type(data['artists']))
# ! there is a dictionary inside of a dictionary.


# now we want to get the key from this inner dictionary
print(data['artists'].keys())

# we can also store this as a variable
artist_data = data['artists']

# same outcome as if we hadn't used brackets
print(artist_data.keys())


print(artist_data['next'])
#this returns a url

print(artist_data['href'])
# returns another url

print(artist_data['limit'])
# returns a number

print(artist_data['items'])
print(type(artist_data['items']))
# this is a list instead of a dictionary!

print(artist_data['items'][0])
...
# ooh here's a picture of drake that I found on this call
...
print(artists[1].keys())
print(artists[1]['name'])

print(artists[1]['name'], "has a popularity of", artists[1]['popularity'])

print(artists[2].keys())
print(artists[2]['name'])

print(artists[2]['name'], "has a popularity of", artists[2]['popularity'])

print("There are",len(artists), "named Drake")



# this loops through the information for every Drake!
# woohoo
# this shows that Spotify does not sort the Drakes by their popularity rating.
...
# uniform resource indictor
# like a URL but not tied to the web
print(real_drake['uri'])


print(real_drake['images'])

# this is the "best" image of Drake
...
# looping through all of the images in the images dictionary
for image in real_drake['images']:
    # print (type(image))
    # print(image.keys())
    print(image['url'])
    # upon further examination, they are all the same image ! # but different sizes 
